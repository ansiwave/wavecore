...
proc selectPost*(conn: PSqlite3, sig: string): Post =
  let query =
    """
      SELECT post_id, ts, content, content_sig, content_sig_last, public_key, parent, reply_count, score, partition, tags, extra_tags, extra_tags_sig, extra_tags_editor FROM post
      WHERE content_sig = ?
    """
